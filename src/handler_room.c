...
void affect_check_room(ROOM_INDEX_DATA *room,int where,int vector)
 ...
 bool is_safe_rspell(int level, CHAR_DATA *victim)
 {
   if (is_safe_rspell_nom(level,victim))
     {
       act("Tanrılar $m koruyor.",victim,NULL,NULL,TO_CHAR);
       act("Tanrılar $m odanın büyüsünden koruyor.",victim,NULL,NULL,TO_ROOM);
       return TRUE;
     }
   else return FALSE;
 }
 ...
 void raffect_to_char( ROOM_INDEX_DATA *room, CHAR_DATA *ch)
  {
   ...
   if ( IS_ROOM_AFFECTED( room, AFF_ROOM_L_SHIELD) )
   {
       ...
       if (!is_safe_rspell(paf->level,ch))
 	{
          ...
          {
            send_to_char("Enerji düşmanlarında yitip gidiyor.\n\r",ch);
   ...
   {
     send_to_char("Birisi tarafından kurulmuş tuzak seni engelliyor.\n\r",ch);
   ...
   send_ch_color("$CHavayı dolduran gizemi hissediyorsun.$c\n\r",ch,POS_SLEEPING,CLR_YELLOW);
