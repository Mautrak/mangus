...
void spell_identify( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    sprintf( buf,
      "Obje '%s', tip %s, materyal %s, ekstra �zellik %s.\n\rA��rl�k %d gr, de�er %d, seviye %d.\n\r",
    ...
    case ITEM_CONTAINER:
    sprintf(buf,"Kapasite: %d#  Maks. tek e�ya a��rl���: %d#  �zellikler: %s\n\r",
            ...
            if (paf->bitvector)
            {
                switch(paf->where)
                {
                    case TO_AFFECTS:
                        ...
                        break;
                    case TO_OBJECT:
                        printf_to_char(ch,"%s e�ya �zelli�i ekler.\n\r",extra_bit_name(paf->bitvector));
                        break;
                    case TO_WEAPON:
                        printf_to_char(ch,"%s silah �zelli�i ekler.\n\r",weapon_bit_name(paf->bitvector));
                        break;
                    case TO_IMMUNE:
                        printf_to_char(ch,"%s ba����kl��� ekler.\n\r",imm_bit_name(paf->bitvector));
                        break;
                    case TO_RESIST:
                        printf_to_char(ch,"%s direnci ekler.\n\r",imm_bit_name(paf->bitvector));
                        break;
                    case TO_VULN:
                        printf_to_char(ch,"%s dayan�ks�zl��� ekler.\n\r",imm_bit_name(paf->bitvector));
                        break;
                    case TO_DETECTS:
                        printf_to_char(ch,"%s saptamas� ekler.\n\r",detect_bit_name(paf->bitvector));
                        break;
                    default:
