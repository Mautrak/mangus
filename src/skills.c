...
void do_gain(CHAR_DATA *ch, char *argument)
{
    ...
    if (arg[0] == '\0')
    {
      do_say(trainer,(char*)"10 prati�i 1 e�itime d�n��t�rebilirsin.");
    	do_say(trainer,(char*)"1 e�itimi 10 prati�e d�n��t�rebilirsin.");
    	do_say(trainer,(char*)"�unlar� kullan: 'kazan e�itim','kazan pratik'");
	return;
    }
	...
	if (ch->train < 1)
	{
    act("$N anlat�yor 'Haz�r de�ilsin.'",
		ch,NULL,trainer,TO_CHAR);
	    return;
	}

  act("$N e�itimini pratiklere d�n��t�rmene yard�m ediyor.",
		ch,NULL,trainer,TO_CHAR);
	ch->practice += 10;
	ch->train -=1 ;
	return;
  ...
  act("$N pratiklerini e�itime d�n��t�rmene yard�m ediyor.",
		ch,NULL,trainer,TO_CHAR);
	ch->practice -= 10;
	ch->train +=1 ;
	return;
    ...
    act("$N anlat�yor 'Anlamad�m...'",ch,NULL,trainer,TO_CHAR);
...
void check_improve( CHAR_DATA *ch, int sn, bool success, int multiplier )
{
	...
	{
	    sprintf(buf,
        "$CHatalar�ndan ders al�yorsun ve %s geli�iyor.$c",
		skill_table[sn].name[1]);
	    act_color(buf,ch,NULL,NULL,TO_CHAR,POS_DEAD,CLR_GREEN);
...
void do_slist(CHAR_DATA *ch, char *argument)
{
    ...
    if (arg[0] == '\0')
	{
    send_to_char("Yaz�m: syetenek <s�n�f>.\n\r",ch);
	 return;
	}
    iclass = class_lookup(arg);
    if (iclass == -1)
	{
    send_to_char("Ge�erli bir s�n�f de�il.\n\r",ch);
	 return;
	}
    /* initilize data */
    for (lev = 0; lev < LEVEL_HERO; lev++)
    ...
    if (!found)
    {
      send_to_char("O s�n�fta yetenek yok.\n\r",ch);
      return;
    }
...
void do_glist( CHAR_DATA *ch , char *argument)
{
 ...
 if (arg[0] == '\0')
  {
    send_to_char("Yaz�m : grupliste <grup>\n\r",ch);
   return;
  }

 if ((group = group_lookup(arg) ) == -1)
  {
    send_to_char("Ge�erli bir grup de�il.\n\r",ch);
   return;
  }
...
void do_slook( CHAR_DATA *ch, char *argument)
{
     ...
     if (arg[0] == '\0')
	{
	 send_to_char("Yaz�m : slook <skill or spell name>.\n\r",ch);
	 return;
	}

     if ( (sn = skill_lookup(arg)) == -1 )
	{
	 send_to_char("B�yle bir b�y� veya yetenek yok.\n\r",ch);
	 return;
	}
...
void do_learn( CHAR_DATA *ch, char *argument )
{
	...
	if ( argument[0] == '\0')
	{
    send_to_char( "Yaz�m: ��ren <yetenek|b�y�> <��retici>.\n\r", ch );
	    return;
	}
...
(skill_table[sn].cabal != ch->cabal && skill_table[sn].cabal != CABAL_NONE) )))
	...
	if ( sn == gsn_vampire )
	{
    send_to_char( "Bu konuda ancak g�revci yard�m edebilir.\n\r",ch);
	 return;
	}
	...
	{
    send_to_char( "Kahraman�n burada de�il.\n\r", ch );
	    return;
	}

	if ( IS_NPC(mob) || mob->level != HERO )
	{
    send_to_char( "Bir kahraman bulmal�s�n, s�radan birini de�il.\n\r",ch);
	  return;
	}

	if ( mob->status != PC_PRACTICER )
	{
    send_to_char( "Kahraman�n ��retmeye istekli g�r�nm�yor.\n\r",ch);
	  return;
	}

	if (get_skill(mob,sn) < 100)
	{
    send_to_char("Kahraman�n bu konuyu ��retebilecek kadar bilmiyor.\n\r",ch);
	  return;
	}
	...
	{
    sprintf( buf, "%s konusunu zaten ��rendin.\n\r",
		skill_table[sn].name[1] );
	    send_to_char( buf, ch );
	}
	else
...
void do_teach( CHAR_DATA *ch, char *argument)
{
 if (IS_NPC(ch) || ch->level != LEVEL_HERO)
  {
    send_to_char("Bir kahraman olmal�s�n.\n\r",ch);
	return;
  }
 ch->status = PC_PRACTICER;
 send_to_char("Art�k %100 olan yeteneklerini gen�lere ��retebilirsin.\n\r",ch);
 return;
}
