...
int main( int argc, char **argv )
{
...
#if defined(macintosh) || defined(MSDOS)
    boot_db( );
    log_string( "Mangus kullan�ma haz�r." );
    game_loop_mac_msdos( );
#endif
    ...
    boot_db( );
		sprintf( log_buf, "Mangus %d portunda kullan�ma haz�r.", port );
    log_string( log_buf );
...
case CON_GET_OLD_PASSWORD:
	...
	if (!str_cmp(arg,"help"))
	{
		...
		do_help(ch,argument);
            write_to_buffer(d,
							"S�n�f�n�z nedir (bilgi i�in: www.mangusmud.com)? ",0);
	    return;
	}
...
"Mangus Mud %d farkl� �rka ev sahipli�i yapar. Irklar�n �zeti:",
	...
	sprintf(buf,
"Bildi�in gibi Mangus Mud'da %d farkl� �rk bulunmaktad�r:",
			MAX_PC_RACE - 1);
	write_to_buffer( d, buf, 0);
	write_to_buffer( d, "\n\r", 0);
	do_help(ch,(char*)"�rklar");
	...
	if (!str_cmp(arg,"yard�m"))
	{
		...
		do_help(ch,argument);
                write_to_buffer(d,
									"Irk�n�z nedir? (bilgi: www.mangusmud.com) ",0);
	      }
	    break;
  	}
	...
	write_to_buffer( d, buf, 0 );
            write_to_buffer(d,
		"S�n�f�n ne olsun (bilgi: www.mangusmud.com)? ",0);
	...
	if ( !ch->endur )
	 {
	  switch(argument[0])
	   ...
	   default:
	    ...
	    write_to_buffer(d, "Eti�in ne olsun, (T/Y/K) (bilgi: www.mangusmud.com)?",0);
	    return;
	   }
         }
	else
    ...
    case CON_READ_MOTD:
	write_to_buffer( d,
    "\n\rMangus'a ho�geldin. �l�m�n tad�n� ��kar!!...\n\r",
	    0 );
	ch->next	= char_list;
	char_list	= ch;
	...
	{
		printf_to_char( ch , "\n\r{CDiscord ID'niz kay�tl� de�il. 'discord' komutuyla discord kullan�c� ID'nizi kaydediniz.{x\n\r\n\r" );
	}
