...
#define PERS(ch, looker)	( can_see( looker, (ch) ) ?         \
				( IS_NPC(ch) ? (ch)->short_descr :((IS_VAMPIRE(ch) && !IS_IMMORTAL(looker))? "�irkin bir varl�k" : (ch)->name ) ) \
				: ( (!IS_NPC(ch) && (ch)->level > (LEVEL_HERO)) ? "bir �l�ms�z" : "birisi" ) )
/* new defines */
...
/* ek.c */
char *	ekler args( (CHAR_DATA *to, CHAR_DATA *ch, char *format) );
