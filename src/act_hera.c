...
void do_enter( CHAR_DATA *ch, char *argument)
{
	...
	if (portal == NULL)
	{
    printf_to_char(ch,"�yle bir�ey g�rm�yorsun.\n\r");
	    return;
	}
	...
	{
    printf_to_char(ch,"Bir yol bulam�yorsun.\n\r");
	    return;
	}
	...
	{
    printf_to_char(ch,"Bir�ey ayr�lman� engelliyor...\n\r");
	    return;
	}
	...
	{
    act("$p hi�bir yere ula�m�yor.",ch,portal,NULL,TO_CHAR);
	   return;
	}
        ...
        if (MOUNTED(ch))
        sprintf(buf,"$n s�rd��� %s ile $p i�ine giriyor.",MOUNTED(ch)->short_descr );
        sprintf(buf,"$n $p i�ine giriyor." );
	act(buf,ch,portal,NULL,TO_ROOM);

	if (IS_SET(portal->value[2],GATE_NORMAL_EXIT))
  act("$p i�ine giriyorsun.",ch,portal,NULL,TO_CHAR);
	else
  act("$p i�ine y�r�y�nce kendini ba�ka yerde buluyorsun...",ch,portal,NULL,TO_CHAR);
	...
	if (IS_SET(portal->value[2],GATE_NORMAL_EXIT))
	 {
	  if (mount)
    act("$n $M s�rerek geldi.",ch,portal,mount,TO_ROOM);
    else  act("$n geldi.",ch,portal,NULL,TO_ROOM);
	 }
	else
	 {
	  if (mount)
    act("$n $p i�inden $M s�rerek geldi.",ch,portal,mount,TO_ROOM);
  else  act("$n $p i�inden geldi.",ch,portal,NULL,TO_ROOM);
	 }
  ...
  act("$p kayboluyor.",ch,portal,NULL,TO_ROOM);
    ...
    printf_to_char(ch,"Hay�r, bunu yapamazs�n.\n\r");
    return;
}
...
void do_settraps( CHAR_DATA *ch, char *argument )
{
    ...
    &&   ch->level < skill_table[gsn_settraps].skill_level[ch->iclass] )
      {
        printf_to_char(ch,"Tuzak kurmay� bilmiyorsun.\n\r");
	return;
      }
    ...
    if ( IS_SET(ch->in_room->room_flags, ROOM_LAW) )
	{
    send_to_char("Mistik bir g�� oday� koruyor.\n\r",ch);
	 return;
	}
    ...
    ||   number_percent( ) <  ( get_skill(ch,gsn_settraps) * 0.7 ) )
    {
      ...
      {
        send_to_char("Bu odaya zaten tuzak kurulmu�.\n\r",ch);
	return;
       }

      if ( is_affected(ch,gsn_settraps))
      {
        send_to_char("Bu yetenek yak�n zamanda kullan�ld�.\n\r",ch);
	return;
      }
...
void do_hunt( CHAR_DATA *ch, char *argument )
{
  ...
  if( arg[0] == '\0' )
    {
      send_to_char( "Kimi avlamaya �al���yorsun?\n\r", ch );
      return;
    }
...
act("$n dikkatle topra�� inceliyor.", ch, NULL, NULL, TO_ROOM );
  ...
  if( direction == -1 )
    {
      act( "Burada $E giden bir yol bulam�yorsun.",
  ...
  if( direction < 0 || direction > 5 )
    {
      send_to_char("Hmm... Bir�eyler hatal� gibi.\n\r", ch );
      return;
    }
  ...
  sprintf( buf, "$N %s y�n�nde.", dir_name[direction] );
  ...
  if( !found || !can_see( ch, ch->hunting ) )
    {
     if( get_char_area( ch, ch->hunting->name) != NULL )
	   ...
	   if( ch->in_room == ch->hunting->in_room )
	    {
        act( "$n $E dik dik bakarak diyor ki, '�leceksin!'",
  	  ch, NULL, ch->hunting, TO_NOTVICT );
        act("$n sana dik dik bakarak diyor ki, '�leceksin!'",
  	  ch, NULL, ch->hunting, TO_VICT );
        act( "$E dik dik bakarak diyorsun ki, '�leceksin!'",
	...
	{
         do_say( ch, (char*)"Ahhhh!  Av�m gitti!!" );
         ch->hunting = NULL;
         return;
        }
...
void damage_to_obj(CHAR_DATA *ch,OBJ_DATA *wield, OBJ_DATA *worn, int damage)
{
 ...
 {
   act_color("$C$p taraf�ndan verilecek hasar� $P engelliyor.$c",ch,wield,worn,TO_ROOM,POS_RESTING,CLR_RED);
   return;
 }
 worn->condition -= damage;

 act_color("$C$p $P �zerine hasar b�rak�yor.$c",
	ch,wield,worn,TO_ROOM,POS_RESTING,CLR_RED);

 if (worn->condition < 1)
	{
    act_color("$C$P par�alara ayr�l�yor.$c",
 ...
 {
   sprintf(buf,"$C$p $P'ye kar�� d�v��mek istemiyor.$c");
   act_color(buf,ch,wield,worn,TO_ROOM,POS_RESTING,CLR_GREEN);
   sprintf(buf,"$C$p kendisini senden kurtar�yor!$c.");
   act_color(buf,ch,wield,worn,TO_CHAR,POS_RESTING,CLR_GREEN);
   sprintf(buf,"$C$p kendisini $z kurtar�yor$c.");
  act_color(buf,ch,wield,worn,TO_ROOM,POS_RESTING,CLR_GREEN);
 ...
 if (IS_SET(wield->extra_flags,ITEM_ANTI_EVIL)
	&& IS_SET(worn->extra_flags,ITEM_ANTI_EVIL))
 {
   sprintf(buf,"$C$p $P �zerine zarar uygulamaktan korkuyor.$c");
  act_color(buf,ch,wield,worn,TO_ROOM,POS_RESTING,CLR_GREEN);
  return;
 }
 return;
...
void do_repair(CHAR_DATA *ch, char *argument)
{
    ...
    if (arg[0] == '\0')
    {
	do_say(mob,(char*)"Senin i�in bir silah� onarabilirim. Tabii �cret kar��l���nda.");
	send_to_char("Type estimate <weapon> to be assessed for damage.\n\r",ch);
	return;
    }
    if (( obj = get_obj_carry(ch, arg)) == NULL)
    {
	do_say(mob,(char*)"Sende bu e�ya yok.");
	return;
    }

    if (obj->pIndexData->vnum == OBJ_VNUM_HAMMER)
    {
     do_say(mob,(char*)"Bu �eki� benim ustal���m�n �st�nde.");
     return;
    }

    if (obj->condition >= 100)
    {
	do_say(mob,(char*)"Bu e�ya zaten iyi durumda.");
        return;
    }
    ...
    if (cost > ch->silver)
    {
	do_say(mob,(char*)"Hizmetlerimden yararlanmak i�in yeterince paran yok.");
	return;
    }
    ...
    ch->silver -= cost;
    mob->silver += cost;
    sprintf(buf, "$N $n'dan %s'� al�yor, tamir ediyor ve $n'a geri veriyor.", obj->short_descr);
    act(buf,ch,NULL,mob,TO_ROOM);
    sprintf(buf, "%s %s'� al�p, tamir edip sana geri veriyor.\n\r", mob->short_descr, obj->short_descr);
    send_to_char(buf, ch);
    obj->condition = 100;
}
...
void do_estimate(CHAR_DATA *ch, char *argument)
{
    ...
    if (arg[0] == '\0')
    {
	do_say(mob,(char*)"�cret <e�ya> yazmay� dene.");
   	return;
    }
    if ((obj = (get_obj_carry(ch, arg))) == NULL)
    {
    ...
    if (obj->cost == 0)
    {
	do_say(mob,(char*)"Bu e�ya tamir edilemeyecek durumda.");
    	return;
    }
    ...
    sprintf(buf, "Bu e�yay� tamir etmek sana %d ak�eye patlar.", cost);
    do_say(mob,buf);
}
...
void do_restring( CHAR_DATA *ch, char *argument )
{
    ...
    if ( mob == NULL )
    {
      send_to_char( "Burada yapamazs�n, bir �ifac� bul.\n\r", ch );
        return;
    }
	...
	{
    send_to_char( "Ad�n� de�i�tirmek istedi�in e�yay� ve de�i�tirmek istedi�in alan� belirt.\n\r", ch );
    send_to_char("Yaz�m:\n\r",ch);
		send_to_char("  isimlendir <e�ya> <alan> <yeni isim>\n\r",ch);
    ...
    if (cost > ch->silver)
    {
      act("$N 'Hizmetlerim i�in yeterli ak�en yok,' dedi.",
		  ch,NULL,mob,TO_CHAR);
		return;
    }
	...
	{
    send_to_char("Ge�erli bir de�il.\n\r",ch);
		return;
	}
    ...
    ch->silver -= cost;
    mob->silver += cost;
    sprintf(buf, "$N $s e�yas�n� al�yor, �zerine bir �eyler kaz�y�p geri veriyor.");
	act(buf,ch,NULL,mob,TO_ROOM);
  sprintf(buf,"%s e�yan� al�yor, �zerine bir �eyler kaz�y�p geri veriyor.\n\r", mob->short_descr);
  send_to_char(buf,ch);
  send_to_char("E�ya isimlendirmede sald�rgan ve hakaret i�eren isimler vermek kurallara ayk�r�d�r.\n\r", ch);
}
...
void do_smithing(CHAR_DATA *ch, char *argument)
{
   ...
   if (obj->condition >= 100)
    {
      send_to_char("E�yan zarar g�rmemi�.\n\r",ch);
	return;
    }

    if (( hammer = get_hold_char(ch)) == NULL)
    {
      send_to_char("Bir �eki� ta��m�yorsun.\n\r",ch);
	return;
    }

    if ( hammer->pIndexData->vnum != OBJ_VNUM_HAMMER )
    {
      send_to_char("Elindeki �eki� uygun de�il.\n\r",ch);
	return;
    }
    ...
    if ( number_percent() > get_skill(ch,gsn_smithing) )
     {
      ...
      sprintf(buf, "$n %s e�yas�n� �eki�le tamir etmeyi denedi fakat beceremedi.", obj->short_descr);
    act(buf,ch,NULL,obj,TO_ROOM);
    sprintf(buf, "%s e�yas�n� tamir etmeyi beceremedin.\n\r", obj->short_descr);
    send_to_char(buf, ch);
    hammer->condition -= 25;
     }
    else
    ...
    sprintf(buf, "$n %s e�yas�n� �eki�le tamir etti.", obj->short_descr);
    act(buf,ch,NULL,NULL,TO_ROOM);
    sprintf(buf, "%s e�yas�n� tamir ettin.\n\r", obj->short_descr);
    send_to_char(buf, ch);
...
void auction_update (void)
{
    ...
    if (auction->item != NULL)
        if (--auction->pulse <= 0) /* decrease pulse */
        {
            ...
            case 2 : /* going twice */
            if (auction->bet > 0)
            sprintf (buf, "%s: %d ak�eye gidiyor.", auction->item->short_descr,auction->bet);
            else
            sprintf (buf, "%s: hen�z teklif yok.", auction->item->short_descr);
            ...
            if (auction->bet > 0)
            {
              sprintf (buf, "%s: %s %d ak�eye sat�n ald�.",
	        ...
	        sprintf(bufc,"%s%s%s",CLR_CYAN,buf,CLR_WHITE_BOLD);
                ...
                act ("Mezat�� yan�nda belirerek $p e�yas�n� sana veriyor.",
                     auction->buyer,auction->item,NULL,TO_CHAR);
                act ("Mezat�� beliriyor ve $p e�yas�n� $e veriyor.",
            ...
            {
              sprintf (buf, "%s teklif gelmedi�i i�in mezattan �ekildi.",auction->item->short_descr);
			  sprintf(bufc,"%s%s%s",CLR_CYAN,buf,CLR_WHITE_BOLD);
              talk_auction(bufc);
              sprintf (buf, "Mezat�� sat�lmayan e�yay� depoya kald�r�yor.");
...
void do_auction (CHAR_DATA *ch, char *argument)
{
    ...
    if (!str_cmp(arg1,"a��k") )
	 {
     send_to_char("Mezat kanal� a��ld�.\n\r",ch);
	  REMOVE_BIT(ch->comm,COMM_NOAUCTION);
	  return;
	 }
	 else
	 {
     send_to_char("Mezat kanal� kapand�.\n\r",ch);
 	  send_to_char( "Mezat kanal�n a��k de�il.\n\r",ch);
	  return;
	 }
	}
    ...
    if (arg1[0] == '\0')
    {
        if (auction->item != NULL)
        {
            /* show item data here */
            if (auction->bet > 0)
            sprintf (buf, "Bu objeye son verilen teklif %d ak�e.\n\r",auction->bet);
            else
            sprintf (buf, "Hen�z teklif verilmedi.\n\r");
        ...
        {
          printf_to_char(ch,"{rNeyi mezata ��karacaks�n?{x\n\r");
            return;
        }
    }

    if (!str_cmp(arg1,"kapal�") )
	{
    send_to_char("Mezat kanal� kapand�.\n\r",ch);
  ...
  if (IS_IMMORTAL(ch) && !str_cmp(arg1,"durdur"))
    ...
    if (auction->item == NULL)
    {
      send_to_char ("Durdurabilece�in bir mezat y�r�m�yor.\n\r",ch);
        return;
    }
    else /* stop the auction */
    {
      sprintf(buf,"%s objesinin sat��� bir �l�ms�z taraf�ndan durduruldu.",auction->item->short_descr);
	sprintf(bufc,"%s%s%s",CLR_WHITE,buf,CLR_WHITE_BOLD);
        ...
        if (auction->buyer != NULL) /* return money to the buyer */
        {
            ...
            printf_to_char (auction->buyer,"%d ak�en iade edildi.\n\r",auction->bet);
        }
        return;
    }
    }
	...
	if ( ch == auction->seller )
		{
      send_to_char("Kendi mal�n i�in art�r�m yapamazs�n...:)\n\r",ch);
	return;
		}
            ...
            {
              send_to_char ("Teklifin son tekliften 1 ak�e fazla ya da a��l�� fiyat� kadar olmal�.\n\r",ch);
                return;
            }
            ...
            sprintf (buf,"%s i�in %d ak�e teklif edildi.\n\r",auction->item->short_descr,newbet);
        ...
        {
          send_to_char ("Mezatta bir�ey yok.\n\r",ch);
            return;
        }
        }
    ...
    {
      send_to_char ("Onu mezata s�remezsin.\n\r",ch);
        return;
    }
    ...
    for (i=1; i < MAX_CABAL; i++)
    {
      ...
      {
        send_to_char("�ste�in tanr�lar� sinirlendiriyor.\n\r", ch);
	return;
      }
    }
    ...
    default:
    act_color ("$C$T e�yalar� mezata s�r�lemez.$c",ch, NULL, item_type_name(obj),
    ...
    {
      act ("�u an mezatta $p e�yas� var, daha sonra tekrar dene!",ch,auction->item,NULL,TO_CHAR);
        return;
    }
}
