...
static bool bot_parse_line( char *line )
{
    ...
    strcpy( bot->name, capitalize( field[0] ) );
    bot->race       = race;
    bot->iclass     = iclass;
...
void do_botlar( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !str_cmp( arg, "bağla" ) || !str_cmp( arg, "ayır" ) )
    {
        char who[MAX_INPUT_LENGTH];
        bool login = !str_cmp( arg, "bağla" );
    ...
    if ( !str_cmp( arg, "av" ) )
    {
        ...
        bot_debug_areas( ch, bot );
        return;
    }

    if ( ( bot = bot_find( arg ) ) == NULL )
    {
        send_to_char( "Öyle bir bot yok.\n\r", ch );
        return;
    }
    ...
    if ( bot->ch == NULL )
    {
        printf_to_char( ch, "Çevrimdışı; sonraki giriş denemesi %ld sn sonra.\n\r",
                        (long) ( bot->next_login_try - current_time ) );
        return;
    }
    printf_to_char( ch, "Durum %s (%d pulse), alt durum %d, yol %d/%d hedef %d\n\r",
                    bot_state_name( bot->state ), bot_pulse - bot->state_pulse, bot->substate,
                    bot->path_pos, bot->path_len, bot->target_vnum );
    printf_to_char( ch, "Oda %d (%s), bölge %s, av bölgesi %s\n\r",
                    bot->ch->in_room != NULL ? bot->ch->in_room->vnum : 0,
